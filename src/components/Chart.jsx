...
const Chart = () => {
  ...
  return (
    <Box sx={{ width: { xs: "100%" }, mt: 3, p: 2 }}>
      <Grid container spacing={1}>
        {/* First chart - flex grow to fill extra space */}
        <Grid item xs={12} sx={{ flexGrow: 1 }}>
          <Card sx={{ backgroundColor: "#06275f", borderRadius: "16px", color: "#fff", height: "100%", display: "flex", flexDirection: "column", boxShadow: "0 4px 20px rgba(0,0,0,0.5)" }}>
            <CardContent>
        ...
        {/* Second chart - fixed width */}
        <Grid item xs={12} md={5}>
          <Card sx={{ backgroundColor:"#061e45", borderRadius: "16px", color: "#fff", height: "100%", display: "flex", flexDirection: "column", boxShadow: "0 4px 20px rgba(0,0,0,0.5)" }}>
            <CardContent>
              <ReactApexChart options={barOptions} series={barSeries} type="bar" height={180} />
              <Typography variant="h6" sx={{ fontWeight: "bold", mt: 2 }}>Active Users</Typography>
                ...
                {iconOption.map((item, index) => (
                  <Grid item xs={6} key={index}>
                    <Box sx={{ textAlign:"center" }}>
                      <Box sx={{ display: "flex", alignItems: "center", gap: 4,mt: 2, justifyContent:"center" }}>
                        {item.icon}
                        <Typography variant="body2" sx={{ color: "#A0AEC0", fontWeight: "bold" }}>{item.label}</Typography>
                      </Box>
                      <Typography variant="h6">{item.value}</Typography>
                      <Box sx={{ mt:1 }}>
                        <LinearProgress variant="determinate" value={item.progress} sx={{ height: 6, borderRadius: 100, backgroundColor: item.barColors[1], "& .MuiLinearProgress-bar": { backgroundColor: item.barColors[0] } }} />
                      </Box>
                    </Box>
                  </Grid>
                ))}
